...
class Notepad:
    ...
    def set_status_bar2(self):
        self.status_bar2 = ttk.Label(self.root, text='Timer')
        self.status_bar2.pack(side=tk.TOP)
        self.count2=0
        self.show_statusbar2=tk.BooleanVar()
        self.show_statusbar2.set(True)
    ...
    def counter_label(self):
        def count():
                ...
                display=string
                self.status_bar2.config(text=display)
                self.status_bar2.after(1000, count)
                self.status_bar2.pack(anchor='center')
                counter += 1
        count()
    ...
    def Start(self):
        global running
        running = True
        string=self.counter_label()
        self.status_bar2.config(text=string)
        self.status_bar2.after(1000, self.start)
        self.status_bar2.pack(anchor='nw')

    def time_used(self):
        self.status_bar2.config(text='Time used :')
        self.status_bar2.pack(anchor='center')

    def time(self):
        string = time.ctime()
        self.status_bar.config(text = string)
        self.status_bar.after(1000,self.time)
    ...
    def open_file(self, event=None):
        try:
            self.msg,self.base=self.file_controller.read_file(url="")
            if self.db_controller.is_file_secure(self.base):
                ...
                if pwd==self.db_controller.get_file_pwd(self.base):
                    ...
                    self.root.title(self.base)
                     ...
                     messagebox.showerror("Wrong File Name", "File does not exist!")
        except:
                     print(traceback.format_exc())
    ...
    def say_something(self):
        try:
            ...
            messagebox.showerror("Non transable text", "Speech unrecognizable")
...
##load file from DB
    ...
    def open_secure_file(self):
       try:
            ...
            if len(self.sel_file_index_tuple)==0:
                ...
                if self.db_controller.is_file_secure(basename):
                    pwd=self.get_file_pwd()
           ...
           messagebox.showerror("DB Error", "Error while opening file")
           print(traceback.format_exc())
    ...
    def threading(self):
        threading.Thread(target=self.time_used).start()
        threading.Thread(target=self.counter_label).start()
